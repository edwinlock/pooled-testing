...
const PILOT_BUDGETS     = collect(2:4:26)  # {2, 6, ..., 26} — pilot experiments
const SYNTHETIC_BUDGETS = [2,4,8,12]       # {2, 4, 8, 12} — synthetic experiments
const SYNTHETIC_N       = 150              # synthetic population size
const SYNTHETIC_REPS    = 50               # synthetic populations per experiment
...
# by accuracy_params; override MILP_K when set in an experiment spec). Each
# experiment sets its own `accuracy` in EXPERIMENT_SPECS below; these are the
# defaults to reference there.
const PILOT_ACCURACY     = 0.002           # 0.2% — default for the pilot experiments
const SYNTHETIC_ACCURACY = 0.002           # 0.2% for the synthetic experiments
...
# Each experiment carries its own accuracy so they can be loosened independently
# (e.g. exp 2 at 0.5% to reach high budgets the 0.2% solve can't finish).
